...
import minerful.simplification.SubsumptionChecker.Subsumption.Equalisation;
import minerful.simplification.SubsumptionChecker.Subsumption.Extension;
import minerful.simplification.SubsumptionChecker.Subsumption.None;
import minerful.simplification.SubsumptionChecker.Subsumption.Restriction;
import minerful.simplification.SubsumptionChecker.Subsumption.SubsumptionKind;
import minerful.simplification.SubsumptionChecker.Subsumption.SubsumptionKindClassComparator;
import minerful.simplification.SubsumptionChecker.Subsumption.SubsumptionKindComparator;

public class SubsumptionChecker {
	...
	public SubsumptionChecker(Constraint[] model) {
		this.initCounters();
		this.model = model;
	}
	
	public SubsumptionChecker(Collection<Constraint> allConstraints) {
		this.initCounters();
	...
	public Subsumption checkSubsumption(Constraint c) {
		...
		for (int i = 0; i < model.length && subsumption == null; i++) {
			...
			if (modelCon.equals(c)) {
				...
				CouplingRelationConstraint coCon = ((CouplingRelationConstraint)c);
				if (coCon.getPlausibleForwardConstraint().equals(modelCon)) {
					subsumption = new Subsumption(modelCon, Restriction.INCLUDES_AS_FORWARD);
				}
				if (coCon.getPlausibleBackwardConstraint().equals(modelCon)) {
				...
				CouplingRelationConstraint coCheckCon = ((CouplingRelationConstraint)modelCon);
				if (coCheckCon.getPlausibleForwardConstraint().equals(c)) {
					subsumption = new Subsumption(modelCon, Extension.IS_FORWARD_OF);
				}
				if (coCheckCon.getPlausibleBackwardConstraint().equals(c)) {
