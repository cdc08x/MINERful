...
MAINCLASS="minerful.MinerFulSimuSubCheckStarter"
...
echo "#"
echo "# Test the impact of the support threshold and of the branching factor"
echo "# Branching factor: from $min_branch to $max_branch"
echo "# Support threshold: ${supportThreshold[*]}"
echo "#"
  ...
  for (( sThre=0; sThre<${#supportThreshold[*]}; sThre++ ))
   do
    for (( i=1; i<=$REPETITIONS_PER_COMBO; i++ ))
     do
      n=`expr $n + 1`
      csvout=`java -Xmx$MEMORY_MAX -cp ../MINERful.jar $MAINCLASS -a $default_alphabet -d $DEBUGLEVEL -m $default_tracelength_min -M $default_tracelength_max -s $default_testbed_size -t ${supportThreshold[sThre]} -b $b -R -r $default_regexps | grep 'SUB;'`
     echo "$q;$i;$j;$p;$b;${supportThreshold[sThre]};$csvout"
    done
   j=`expr $j + 1`
 done
done
